...
#    if [ ! -z $(grep "$STRING" "$FILE") ]; then exist="true"; else exist="false"; fi
#    echo "$exist";
#     if [ ! -z $(grep "$STRING" "$FILE") ]; then
#        echo "xdebug was configured already. if you run into problems, please remove all xdebug extensions from your php.ini before dockering up"
#     else
     ...
     FULLSTRING=echo "$STR1" | echo "$machineIp" | echo "$STR2"
